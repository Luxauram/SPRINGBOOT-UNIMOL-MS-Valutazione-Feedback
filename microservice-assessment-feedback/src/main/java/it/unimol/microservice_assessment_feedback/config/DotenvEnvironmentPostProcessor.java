...
public class DotenvEnvironmentPostProcessor implements EnvironmentPostProcessor, Ordered {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        System.out.println("MS-ASSESSMENT: EnvironmentPostProcessor - Tentativo di caricamento file .env...");
        ...
        for (String path : paths) {
            try {
                ...
                System.out.println("MS-ASSESSMENT: Tentativo caricamento da: " + path + " (assoluto: " + absolutePath + ")");

                File envFile = new File(dir, ".env");
                System.out.println("MS-ASSESSMENT: Cercando file: " + envFile.getAbsolutePath() + " - Esiste: " + envFile.exists());
        ...
        if (!envLoaded) {
            try {
                ...
                if (workingDir.contains("microservice-assessment-feedback")) {
                    projectRoot = workingDir.substring(0, workingDir.lastIndexOf("microservice-assessment-feedback"));
                } else {
                ...
                if (projectRoot != null) {
                    ...
                    System.out.println("MS-ASSESSMENT: Tentativo percorso progetto: " + envFile.getAbsolutePath() + " - Esiste: " + envFile.exists());
                    ...
                    if (envFile.exists() && envFile.canRead()) {
                        ...
                        if (dotenv != null && !dotenv.entries().isEmpty()) {
                            System.out.println("MS-ASSESSMENT: File .env caricato dalla root del progetto: " + projectRoot);
                            envLoaded = true;
                        }
                    }
                }
        ...
        if (envLoaded && dotenv != null) {
            ...
            environment.getPropertySources().addFirst(new PropertiesPropertySource(DOTENV_SOURCE_NAME, props));
            System.out.println("MS-ASSESSMENT: File .env caricato con successo! Totale variabili: " + props.size());
        } else {
            System.out.println("MS-ASSESSMENT: ATTENZIONE - Nessun file .env trovato o caricato!");
            System.out.println("MS-ASSESSMENT: Verifica che il file .env esista in uno di questi percorsi:");
            for (String path : paths) {
                File dir = new File(path);
                File envFile = new File(dir, ".env");
        ...
        environment.getPropertySources().forEach(ps -> {
            System.out.println("  - " + ps.getName() + " (classe: " + ps.getClass().getSimpleName() + ")");
        });
    }
}